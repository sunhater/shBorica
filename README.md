...
## Подготовка
Преди всичко останало е необходимо да имам регистриран в Борика търговец, от който първоначално ми трябват неговото ID (MID) и ID на терминала (TID). С тяхна помощ се създават частните ключове и заявките за сертификат за тестов режим и реална среда.
...
Първоначално банката ще върне само сертификат за тестовия режим. Когато разработката е готова, се попълва една таблица и се прави лог на изпратените и получените данни в тестов режим, за всеки един тип транзакция, който се използва в разработката (обикновено само тип 1 - плащане). Таблицата и логът се изпращат на банката, която след одобрение издава сертификат за реална среда. Таблицата и логът и върнатите сертификаи запазвам в папката, която създадох при подготовката.
